...
agent = Agent(action_space, gamma, minibatch_size, observation_shape, replay_memory_capacity)
...
agent.initialize_neural_net_weights(sess)
...
while completed_episodes < nb_episodes:
    ...
    while episode_done is not True:
        ...
        if number_of_observations_experienced >= replay_start_size:
            ...
            if number_of_observations_experienced % target_network_update_frequency == 0:
                agent.update_target_Q_network_weights(sess)
...
np.save('out/dqn2_toy_reward', np.array(episode_reward))
np.save('out/dqn2_toy_length', np.array(episode_length))